...
import com.cocthulhu.testlib.COCutil.Caculate;
...
public class COCHelper {
    ...
    public static class helper_storage{
        final static String storage_save_path= "/tmp/cocdata";
        ...
        public static void saveGlobalInfo(String key,String value) {//保存全局配置信息
            JSONObject obj=readConfig("Global");
            try {
                obj.put(key,value);
            } catch (JSONException e) {
                e.printStackTrace();
            }
            saveConfig("Global",obj);
        }
        public static String getGlobalInfo(String key){//获取全局配置信息
            JSONObject obj=readConfig("Global",true);
            if(obj==null) {
                try {//默认配置信息
                    obj=new JSONObject("{\n" +
                            ...
                            "\"SENTENCE_PROMOTION_FAILURE\":\"全部木大！\",\n" +
                            "\"SENTENCE_JRRP\":\"恩...要我说的话，你今天大概能遇到%d只小啊呜吧～\",\n" +
                            "\"SENTENCE_BIG_FAILURE\":\"哦豁，那么你骰出了一个煤球并糊在了你的脸上！\\\"凉凉月色为你逆流成河...化作春泥全部木大...\\\"\",\n" +
    ...
    static class helper_constant_data{
        ...
        public static class status{
            ...
            static String[] TI(){
                return new String[]{
                        "失调查员会发现自己只记得最后身处的安全地点，却没有任何来到这里的记忆。例如，调查员前一刻还在家中吃着早饭，下一刻就已经直面着不知名的怪物。这将会持续 %s 轮。",
    ...
    static class helper_calculation{
        ...
        static Result XdXCalculation(String expression,int mode){
            ...
            Pattern expression_pattern = Pattern.compile("(\\d+d\\d+)|(d\\d+)|d");
            Matcher expression_matcher = expression_pattern.matcher(expression);
            ArrayList<Result> XdX= new ArrayList<>();
            //String process="";
            int count=0;
            while (expression_matcher.find()) {
                //Log("count="+count);
                String XdX_string=expression_matcher.group();
                Result result=new Result(0,XdX_string);
                XdX.add(result);
                Matcher XdX_matcher = XdX_pattern.matcher(XdX_string);
                int times=1,maxValue=100;
                if(XdX_matcher.find()){
                    int tmp= helper_calculation.StringToInt(XdX_matcher.group(),1);
                    if(XdX_matcher.find()){//XdX的情况
                        maxValue= helper_calculation.StringToInt(XdX_matcher.group(),1);
                        times=tmp;
                    }else{//dX的情况
                        maxValue=tmp;
                    }
        ...
        //成功判断
        static Result resultJudger(int rand, int ability){
            if(rand>ability){
                if(rand>95){
                    return new Result(JUDGE_BIG_FAILURE,helper_storage.getGlobalInfo("SENTENCE_BIG_FAILURE"));
                }else{
                    return new Result(JUDGE_FAILURE,helper_storage.getGlobalInfo("SENTENCE_FAILURE"));
                }
            }else{
                if(rand<6){
                    return new Result(JUDGE_BIG_SUCCESS,helper_storage.getGlobalInfo("SENTENCE_BIG_SUCCESS"));
                }else if(rand<=ability/5){
                    return new Result(JUDGE_VERY_HARD_SUCCESS,helper_storage.getGlobalInfo("SENTENCE_VERY_HARD_SUCCESS"));
                }else if(rand<=ability/2){
                    return new Result(JUDGE_HARD_SUCCESS,helper_storage.getGlobalInfo("SENTENCE_HARD_SUCCESS"));
                }else{
                    return new Result(JUDGE_SUCCESS,helper_storage.getGlobalInfo("SENTENCE_SUCCESS"));
                }
            }
        }
    ...
    public static class helper_interface_in{
        ...
        String id;//用户标识
        long time;//消息时间
        public helper_interface_in(String msg, String id, long time){
            this.msg=msg;
            this.time=time;
            this.id=id;
        }
    ...
    public static class helper_interface_out{
        ...
        public int istroop;//0(私聊) 1(群聊) 1000(群私聊)
        public boolean isrelay;//是否使用回复形式
        public boolean forcePrivateChat;//是否强制私聊
        helper_interface_out(String msg,int istroop,boolean isrelay){
            this.msg=msg;
            this.istroop=istroop;
            this.isrelay=isrelay;
            this.forcePrivateChat=false;
        }
        helper_interface_out(String msg,int istroop,boolean isrelay,boolean forcePrivateChat){
            this.msg=msg;
            this.istroop=istroop;
            this.isrelay=isrelay;
    ...
    private static class helper_do{
        private static String nn(String id, String inputPlayerName){
            //String inputPlayerName=cmd;
            if(helper_calculation.textIsEmpty(inputPlayerName))
                return nnshow(id);
            JSONObject playerInfo= helper_storage.getAbilityInfo(id,inputPlayerName,true);
            String notice;
            if(playerInfo==null){
                //档位不存在，则视作改名
                String oldPlayerName= helper_storage.getPlayerName(id);//取原来的名字
                JSONObject oldplayerInfo= helper_storage.getAbilityInfo(id,oldPlayerName);//取原来角色的信息
                helper_storage.saveAbilityInfo(id,inputPlayerName,oldplayerInfo);//新名字 老档案
                helper_storage.deletePlayerInfo(id,oldPlayerName);//删除老信息
                notice=String.format(Locale.US,helper_storage.getGlobalInfo("SENTENCE_CHANGE_NAME")+" \n %s 已改名为 %s",oldPlayerName,inputPlayerName);
            }else{
                //档位存在，直接设置
                notice=String.format(Locale.US,helper_storage.getGlobalInfo("SENTENCE_CHANGE_CARD")+" \n 已切换现有档位 %s",inputPlayerName);
            }
            helper_storage.savePlayerName(id,inputPlayerName);
            return notice;
        }
        private static String sc(String id, String cmd,int inputsan){
            String[] sc;
            sc=cmd.split("/",2);
            if(sc.length!=2)
                return helper_storage.getGlobalInfo("SENTENCE_ILLEGAL")+"\n格式错误 无法SanCheck.";
            String success_expression=sc[0];
            String failure_expression=sc[1];
            String playername= helper_storage.getPlayerName(id);
            JSONObject abilities= helper_storage.getAbilityInfo(id,playername);
            int san;
            if(inputsan>0)
                san=inputsan;
            else
            ...
            if(san==0){
                ...
                //random_1_100=1;
                helper_calculation.Result result= helper_calculation.resultJudger(random_1_100,ori_san);
                ...
                try {
                    ...
                    helper_storage.saveAbilityInfo(id,playername,abilities);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
        ...
        private static String stshow(String id, String cmd){
            String action=cmd;
            String playername= helper_storage.getPlayerName(id);
            JSONObject abilities= helper_storage.getAbilityInfo(id,playername);

            StringBuilder strinfo=new StringBuilder();
            strinfo.append(String.format(Locale.US,"%s \n %s的信息:\n",helper_storage.getGlobalInfo("SENTENCE_GET_PAYER_INFO"),playername));
            if("".equals(action)){
        ...
        private static String st(String id, String cmd){
            //情况1 赵怡然-斗殴50魅惑30
            ...
            if(cmd_split.length==2){
                if(helper_calculation.isStartWithNumber(cmd_split[1])){
                    //情况2 增量编辑
                    playername= helper_storage.getPlayerName(id);
                    content=cmd;
                }else{
                    //情况1 带名字的批量设置属性
            ...
            while(m.find()){
                ...
                if(value==null)
                    return helper_storage.getGlobalInfo("SENTENCE_ILLEGAL");
                String action=helper_calculation.abilities_name_translate(m.group(1),abilities_name_translate_json);
            ...
            if(count>0) {
                String SENTENCE_SET_PAYER_INFO=helper_storage.getGlobalInfo("SENTENCE_SET_PAYER_INFO");
                helper_storage.saveAbilityInfo(id, playername, abilities);//设置玩家技能档案信息
                helper_storage.savePlayerName(id,playername);//设置当前玩家名字
                if (count > 5) {
                ...
                resultstr = helper_storage.getGlobalInfo("SENTENCE_ILLEGAL");
            }
            return resultstr;

        }
        private static String help(){
            String prefix=helper_storage.getGlobalInfo("PREFIX");
        ...
        private static String robot(){
            return "本骰为[赵系]骰 Beta，可在纯安卓环境运行\n" +
                    " 个人QQ群: 323956301 \n" +
                    " 作者QQ: 2135983891 \n" +
                    " 开发者:赵怡然\n" +
                    " 保留所有解释权";
        ...
        private static String r(String id,String cmd){
            JSONObject abilities=helper_storage.getAbilityInfo(id);
            if(cmd.contains("db")){
                String db=helper_calculation.abilities_get_db(abilities);
                cmd=cmd.replace("db",String.format(Locale.US,"(%s)",db));
            }
            helper_calculation.Result result= helper_calculation.XdXCalculation(cmd);
            return helper_storage.getGlobalInfo("SENTENCE_ROLL")+"\n"+result.notice+"="+result.number;
        }
        private static String en(String id,String cmd){
            //斗殴 or 斗殴50
            String regEx = "([^0-9]+)(\\d+)?";
            Pattern p = Pattern.compile(regEx);
            Matcher m = p.matcher(cmd);
            String playname= helper_storage.getPlayerName(id);
            String resultstr;
            if(m.find()){
                String action=helper_calculation.abilities_name_translate(m.group(1));
                JSONObject abilities= helper_storage.getAbilityInfo(id,playname);
                ...
                if(random_1_100>value){
                    ...
                    helper_storage.saveAbilityInfo(id,playname,abilities);
                    resultstr=String.format(Locale.US,"%s \n 成长%s成功 %d/%d ，属性变化 %d+%d=%d",helper_storage.getGlobalInfo("SENTENCE_PROMOTION_SUCCESS"),action,random_1_100,value,value,random_1_10,new_value);
                }else{
                    resultstr=String.format(Locale.US,"%s \n 成长%s失败 %d/%d ，没有变化。",helper_storage.getGlobalInfo("SENTENCE_PROMOTION_FAILURE"),action,random_1_100,value);
                }
            }else{
                resultstr="格式错误";
            }
        ...
        private static String del(String id, String cmd) {
            String inputPlayerName=cmd;
            if(helper_calculation.textIsEmpty(inputPlayerName)) {
                inputPlayerName = helper_storage.getPlayerName(id);
            }
            helper_storage.deletePlayerInfo(id,inputPlayerName);
            return String.format("%s已删档",inputPlayerName);
        }
        private static String nnshow(String id) {
            String inputPlayerName = helper_storage.getPlayerName(id);
            return String.format("当前角色名称: %s",inputPlayerName);
        }
        private static String rabp(String id, String cmd, int mode){
            ...
            if(m.find()){
                ...
                if(times>5){
                    return helper_storage.getGlobalInfo("SENTENCE_ILLEGAL_TOO_MUCH");
                }
                String action=helper_calculation.abilities_name_translate(m.group(2));
                String value=m.group(3);
                String vs="进行 "+action+" 鉴定 \n";
                int number_ori= helper_storage.getAbilityInfo(id,playname).optInt(action);
                int number_det=0;
                int number;
                ...
                if(mode>0){//奖励或惩罚
                    ...
                    vs+="] ";
                    helper_calculation.Result result= helper_calculation.resultJudger(random_1_100,number);
                ...
                if(helper_calculation.isNumber(cmd)){
                    int ability_value=helper_calculation.StringToInt(cmd,0);
                    if(ability_value==0){
                        return helper_storage.getGlobalInfo("SENTENCE_ILLEGAL")+"\n无法鉴定，概率为0";
                    }
                    int random_1_100=COCHelper.helper_calculation.getRandomInt(1,100);
                    helper_calculation.Result r=helper_calculation.resultJudger(random_1_100,ability_value);
        ...
        private static String jrrp(String id){
            String dataDate= (String) helper_storage.getBaseInfo(id,"jrrp_date");
            int jrrp= helper_calculation.StringToInt(COCHelper.helper_storage.getBaseInfo(id,"jrrp"),0);
            if(!helper_calculation.Date().equals(dataDate)){
                jrrp = helper_calculation.getRandomInt(1, 100);
                COCHelper.helper_storage.saveBaseInfo(id, "jrrp_date", helper_calculation.Date());
                COCHelper.helper_storage.saveBaseInfo(id, "jrrp", jrrp);
            }
            return String.format(Locale.US,helper_storage.getGlobalInfo("SENTENCE_JRRP"),jrrp);
        }
        private static String coc(String cmd){
            StringBuilder returnstr=new StringBuilder();
            returnstr.append("人物作成：\n");
            int times= helper_calculation.StringToInt(cmd,1);
            if(times>10){
                return helper_storage.getGlobalInfo("SENTENCE_ILLEGAL_TOO_MUCH");
            }
            for(int i=0;i<times;i++){
                ...
                int total=0;
                for(int j=0;j<9;j++){
                    arr[j]= helper_calculation.getRandomInt(30,90);
                    arr[j]=(arr[j]/5)*5;
                    total+=arr[j];
                }
        ...
        private static String name(String cmd) {
            String names;
            if(cmd.contains("en")){
                if(cmd.contains("cn")){
    ...
    public static helper_interface_out cmd(helper_interface_in info){
        ...
        if((System.currentTimeMillis())/1000-info.time>6)
            return null;
        String prefix=helper_calculation.ToDBC(helper_storage.getGlobalInfo("PREFIX"));
        ...
        if(cmd.startsWith(prefix)){
            ...
            cmd=cmd.replace("×","*");
            cmd=cmd.replace("÷","/");
            if(cmd.startsWith("sc")){//对sc命令特许空格
                cmd=cmd.substring(2);
                cmd=cmd.trim();
                ...
                if(saninput.length==2 && helper_calculation.isNumber(saninput[1])){
                    ...
                    cmd=cmd.replace(" ","");
                return new helper_interface_out(helper_do.sc(id, cmd,san), 1, reply);
            }
            cmd=cmd.replace(" ","");
            if(cmd.startsWith("runtime")){
                return new helper_interface_out(helper_do.runtime(),1,reply);
            }else if(cmd.startsWith("robot") || cmd.startsWith("about")){
                return new helper_interface_out(helper_do.robot(),1,reply);
            }else if(cmd.startsWith("nnshow")){
                //cmd=cmd.substring(6);
                return new helper_interface_out(helper_do.nnshow(id),1,reply);
            }else if(cmd.startsWith("stshow")){
                cmd=cmd.substring(6);
                return new helper_interface_out(helper_do.stshow(id,cmd),1,reply);
            }else if(cmd.startsWith("help")){
                //cmd=cmd.substring(4);
                return new helper_interface_out(helper_do.help(),1,reply);
            }else if(cmd.startsWith("name")){
                cmd=cmd.substring(4);
                return new helper_interface_out(helper_do.name(cmd),1,reply);
            }else if(cmd.startsWith("jrrp")){
                return new helper_interface_out(helper_do.jrrp(id),1,reply);
            }else if(cmd.startsWith("stclr")){
                cmd=cmd.substring(5);
                return new helper_interface_out(helper_do.del(id,cmd),1,reply);
            } else if(cmd.startsWith("del")){
                cmd=cmd.substring(3);
                return new helper_interface_out(helper_do.del(id,cmd),1,reply);
            }else if(cmd.startsWith("coc")){
                cmd=cmd.substring(3);
                return new helper_interface_out(helper_do.coc(cmd),1,reply);
            }else if(cmd.startsWith("rap")){//同rp
                cmd=cmd.substring(3);
                return new helper_interface_out(helper_do.rabp(id,cmd,1),1,reply);
            }else if(cmd.startsWith("rab")){//同rb
                cmd=cmd.substring(3);
                return new helper_interface_out(helper_do.rabp(id,cmd,2),1,reply);
                ...
                cmd=cmd.substring(2);
                return new helper_interface_out(helper_do.rabp(id,cmd,0),1,reply);
            }else if(cmd.startsWith("nn")){
                cmd=cmd.substring(2);
                return new helper_interface_out(helper_do.nn(id,cmd),1,reply);
            }else if(cmd.startsWith("en")){
                cmd=cmd.substring(2);
                return new helper_interface_out(helper_do.en(id,cmd),1,reply);
            }else if(cmd.startsWith("st")){
                cmd=cmd.substring(2);
                return new helper_interface_out(helper_do.st(id,cmd),1,reply);
            }else if(cmd.startsWith("rh")){//暗骰
                cmd=cmd.substring(2);
                return new helper_interface_out(helper_do.r(id,cmd),1000,false,true);//发起群私聊
            }else if(cmd.startsWith("li")){
                return new helper_interface_out(helper_do.li(),1,reply);
            }else if(cmd.startsWith("ti")){
                return new helper_interface_out(helper_do.ti(),1,reply);
            }else if(cmd.startsWith("r")){
                cmd=cmd.substring(1);
                return new helper_interface_out(helper_do.r(id,cmd),1,reply);
            }
